			...
			switch($val['type']){	
				...
				case 1:
					...
					if($val['is_view_create'] !== 0){
						//生成模板html
						self::createIndexTpl($applicationInfo,$menuInfo,$val,$fieldList,$actionList); 
					}
				break;
						...
						case 1:
							if(empty($val['sql_query'])){	
								...
								if(!empty($val['fields']) && !empty($val['relate_field']) && !empty($val['relate_table'])){
									$str .= "			\$res = db('".$menuInfo['table_name']."')->field(\$field)->alias('a')->join('".$val['relate_table']." b','a.".$val['fields']."=b.".$val['relate_field']."','left')->where(\$where)->order(\$order)->paginate(['list_rows'=>\$limit,'page'=>\$page]);\n";
								}else{
									$str .= "			\$res = ".getControllerName($menuInfo['controller_name'])."::where(\$where)->field(\$field)->order(\$order)->paginate(['list_rows'=>\$limit,'page'=>\$page]);\n";
								}
								$str .= "		}catch(\Exception \$e){\n";
								...
								$str .= "		}\n";
								$str .= "		return ['rows'=>\$res->items(),'total'=>\$res->total()];\n";
								$str .= "	}\n\n\n";	
							}
...
";
			...
			switch($val['type']){
				...
				case 18:
					if($val['is_controller_create'] !== 0){
						...
						if(empty($val['remark'])){
							$str .="			\$res = \utils\sms\JuheSmsService::sendSms(\$data);\n";
						}else{
					...
					switch($val['type']){
						...
						case 1:
							if(empty($val['sql_query'])){	
								...
								$str .= "		try{\n";
								if(!empty($val['fields']) && !empty($val['relate_field']) && !empty($val['relate_table'])){
									$str .= "			\$res = db('".$menuInfo['table_name']."')->field(\$field)->alias('a')->join('".$val['relate_table']." b','a.".$val['fields']."=b.".$val['relate_field']."','left')->where(\$where)->order(\$orderby)->paginate(['list_rows'=>\$limit,'page'=>\$page]);\n";
								}else{
									$str .= "			\$res = ".getControllerName($menuInfo['controller_name'])."::where(\$where)->field(\$field)->order(\$orderby)->paginate(['list_rows'=>\$limit,'page'=>\$page]);\n";
								...
								$str .= "		}\n";
								$str .= "		return ['list'=>\$res->items(),'count'=>\$res->total()];\n";
								$str .= "	}\n\n\n";	
							}
						break;
	...
	public function createModel($actionList,$applicationInfo,$menuInfo){
		...
		$str .="use think\Model;\n";
		$actionList = Action::where(['menu_id'=>$menuInfo['menu_id']])->select();
	...
	public function createIndexTpl($applicationInfo,$menuInfo,$actionInfo,$fieldList,$actionList){
		...
		foreach($show_fields as $k=>$v){
			if(in_array($v['list_show'],[1,2])){
				...
				if(!empty($v['config']) || in_array($v['type'],[7,8,9,10,12,17,22,25])){
					if($v['type'] == 17){
		...
		foreach($scriptGroup as $k=>$v){
			...
			switch($v['type']){
				...
				case 3:
					...
					$htmlstr .= "		var index = layer.open({type: 2,title: '".$v['block_name']."',area: ['".$width."', '".$height."'],fix: false, maxmin: true,content: Feng.ctxPath + '/".getUrlName($menuInfo['controller_name'])."/".$v['action_name']."'+url});\n";
				...
				case 4:
					list($width,$height) = explode('|',$v['remark']);
					$htmlstr .= "		if(value){\n";
					$htmlstr .= "			var index = layer.open({type: 2,title: '".$v['block_name']."',area: ['".$width."', '".$height."'],fix: false, maxmin: true,content: Feng.ctxPath + '/".getUrlName($menuInfo['controller_name'])."/".$v['action_name']."?".$menuInfo['pk_id']."='+value});\n";
					$htmlstr .= "			if(!IsPC()){layer.full(index)}\n";
					$htmlstr .= "		}else{\n";
					...
					$htmlstr .= "				var index = layer.open({type: 2,title: '".$v['block_name']."',area: ['".$width."', '".$height."'],fix: false, maxmin: true,content: Feng.ctxPath + '/".getUrlName($menuInfo['controller_name'])."/".$v['action_name']."?".$menuInfo['pk_id']."='+idx});\n";
					$htmlstr .= "				this.layerIndex = index;\n";
					$htmlstr .= "				if(!IsPC()){layer.full(index)}\n";
					$htmlstr .= "			}\n";
					$htmlstr .= "		}\n";
				...
				case 6:
					...
					$htmlstr .="					});\n";
					$htmlstr .="					ajax.set('".$menuInfo['pk_id']."', idx);\n";
					$htmlstr .="					ajax.set('statusData', value);\n";
					$htmlstr .="					ajax.start();\n";
					$htmlstr .="				});\n";
					$htmlstr .="			}\n";
					$htmlstr .="		}\n";
				...
				//导入数据
				case 13:
					$htmlstr .= "		var index = layer.open({type: 2,title: '".$v['block_name']."',area: ['500px', '300px'],fix: false, maxmin: true,content: Feng.ctxPath + '/".getUrlName($menuInfo['controller_name'])."/".$v['action_name']."'});\n";
					$htmlstr .= "		this.layerIndex = index;\n";
					$htmlstr .= "		if(!IsPC()){layer.full(index)}\n";
				break;
			...
			if(in_array($val['field'],explode(',',$actionInfo['fields']))){
				...
				switch($val['type']){				
					...
					case 9:
						$fieldval = "\n";
						$fieldval .= "						<ul>\n";
						$fieldval .= "						<?php \$imgList = json_decode(html_out(\$info[\"".$val['field']."\"]),true);?>\n";
						$fieldval .= "						{foreach name=\"imgList\" id=\"vo\"}\n";
	...
	public function createValidate($actionList,$applicationInfo,$menuInfo){
		...
		foreach($fieldList as $k=>$v){
			if((!empty($v['validate']) || !empty($v['rule'])) && !in_array($v['type'],[7,12,15,21,25,26,30])){
				$rule .="		";
